...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll()
				.antMatchers("/login**", "/webjars/**", "/error**", "/actuator/health", "/favicon.ico").permitAll()
				.antMatchers("/u/**").authenticated().and().oauth2Login().authorizationEndpoint().and().userInfoEndpoint()
				.customUserType(User.class, "openkx").and().and().exceptionHandling()
	...
	public void configure(WebSecurity web) throws Exception {
		...
		web.ignoring().antMatchers("/css/**", "/js/**", "/fonts/**", "/images/**", "/android");
	}
